...
public class BiliCookiesStorage(
    ...
    public suspend fun importCookies(
        ...
        cookies.forEach { putCookieUnlocked(resolveRequestUrl(requestUrl, it), it) }
    }
    ...
    public fun exportCookies(): List<Cookie> = runBlocking {
        mutex.withLock {
            snapshot.toList()
        }
    }
    ...
    private suspend fun putCookie(requestUrl: String, cookie: Cookie) {
        mutex.withLock {
            putCookieUnlocked(requestUrl, cookie)
        }
    }

    private suspend fun putCookieUnlocked(requestUrl: String, cookie: Cookie) {
